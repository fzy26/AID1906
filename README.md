...
缺点 ：  数据一致性差,数据查找修改不方便(数据量大的时候),数据冗余(大量重复数据)度可能比较大

【3】数据库管理阶段(**DBA  (职业)数据管理员**)
...
>数据库 ： 按照数据结构，存储管理数据的仓库。数据库是在数据库管理系统管理和控制下，在一定介质(存储器)上的数据集合。
...
> MySQL、**Oracle** 、SQL_Server、DB2 SQLite(小型数据库)  
...
>>启动服务：sudo /etc/init.d/mysql start | stop | restart(重启)
...
> 什么是SQL
>
> >结构化查询语言(Structured Query Language)，一种特殊目的的编程语言，是一种数据库查询和程序设计语言，**用于存取数据以及查询、更新和管理关系数据库系统。(操作数据库)**
...
2.创建库(指定字符集)
>create database 库名(参数,区分大小写) [character set utf8];
...
char(参数<255)  具体位数   已经确定有多少
varchar(必须要写  最多 65535)   具体位数可以根据填写的多少可变
...
>>* 如果你想设置数字为**无符号(没有负数)**则加上 unsigned
...
**LIKE**用于在where子句中进行模糊查询，SQL LIKE 子句中使用**百分号 %来**表示任意0个或多个字符，**下划线_表**示任意一个字符。
...
mysql中对正则表达式的支持有限，只支持部分正则元字符**REGEXP**
...
select * from class_1 where sex ='m' ORDER by score desc;所有男生按照成绩降序排列
...
LIMIT [num]
```
eg: select * from class_1 where sex ='m' ORDER by score desc;
...
2. 创建游标对象(cur = db.cursor())--(操作数据库语句,获取查询结果)
